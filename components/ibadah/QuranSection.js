...
export default function QuranSection({ uid }) {
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      await updateDoc(docRef, { quranEntries: arrayUnion(entry) }).catch(async () => {
        const { setDoc } = await import("firebase/firestore");
        await setDoc(docRef, { quranEntries: [entry] }, { merge: true });
      });
      setForm(emptyForm);
      setOpen(false);
    } finally {
  ...
  async function handleDelete(entry) {
    await updateDoc(docRef, { quranEntries: arrayRemove(entry) });
  }
    ...
    <div style={{ background: "var(--surface)", borderRadius: 16, padding: "1.5rem", border: "1px solid var(--border)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
        <h2 style={{ fontSize: "1.1rem", fontWeight: 700, color: "var(--text)", display: "flex", alignItems: "center", gap: "0.5rem" }}>
          <BookMarked size={18} color={amber} />
          Quran
          {totalPages > 0 && (
            <span style={{ background: amber + "22", color: amber, fontSize: "0.7rem", fontWeight: 700, padding: "0.15rem 0.5rem", borderRadius: 20 }}>
              {totalPages}p
            </span>
          )}
          {entries.length > 0 && (
      ...
      {/* Add form */}
      {open && (
        <form onSubmit={handleAdd} style={{ background: "var(--bg)", borderRadius: 12, padding: "1.25rem", border: "1px solid var(--border)", marginBottom: "1.25rem" }}>
          {/* Action toggle */}
          <div style={{ marginBottom: "0.875rem" }}>
            <p style={labelStyle}>Action</p>
            <div style={{ display: "flex", gap: "0.4rem" }}>
              {[
                { key: "read",   label: "Read",   Icon: BookOpen,   color: amber },
                { key: "listen", label: "Listen", Icon: Headphones, color: sky   },
          ...
          <div style={{ marginBottom: "0.875rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.4rem" }}>
              {[
                ...
                { key: "surah-ayah", label: "Surah & Ayah", Icon: Hash     },
              ].map(({ key, label, Icon }) => {
          ...
          {/* Fields by track type */}
          {form.trackType === "page" ? (
            <div style={{ marginBottom: "0.75rem" }}>
              <label style={labelStyle}>Pages <span style={{ color: "var(--danger)" }}>*</span></label>
              <input type="number" min="1" value={form.pages} onChange={(e) => setForm({ ...form, pages: e.target.value })} required placeholder="e.g. 5" style={inputStyle} />
            </div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: "0.625rem", marginBottom: "0.75rem" }}>
              <div>
                <label style={labelStyle}>Surah <span style={{ color: "var(--danger)" }}>*</span></label>
                <input value={form.surah} onChange={(e) => setForm({ ...form, surah: e.target.value })} required placeholder="e.g. Al-Baqarah" style={inputStyle} />
              </div>
              <div>
          ...
          <div style={{ marginBottom: "1rem" }}>
            ...
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} placeholder="Reflection, tafsir notes…" style={{ ...inputStyle, resize: "vertical", height: "auto", fontFamily: "inherit" }} />
          </div>
      ...
      {entries.length === 0 && !open ? (
        <p style={{ color: "var(--muted)", fontSize: "0.875rem", textAlign: "center", padding: "1rem 0" }}>No Quran entries today</p>
      ) : (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", flexWrap: "wrap" }}>
                      ...
                      <span style={{ color: "var(--muted)", fontSize: "0.72rem" }}>
                        {entry.trackType === "page" ? "Pages" : "Surah & Ayah"}
                      </span>
                    </div>

                    {entry.trackType === "page" ? (
                      <p style={{ color: "var(--text)", fontWeight: 700, fontSize: "1rem" }}>
                        {entry.pages} <span style={{ color: "var(--muted)", fontWeight: 400, fontSize: "0.85rem" }}>pages</span>
                      </p>
                    ) : (
                      <p style={{ color: "var(--text)", fontWeight: 600, fontSize: "0.925rem" }}>
                        {entry.surah}
                    ...
                    {entry.notes && (
                      <p style={{ color: "var(--muted)", fontSize: "0.8rem", marginTop: "0.375rem", lineHeight: 1.55 }}>{entry.notes}</p>
                    )}
                  </div>
                  <button onClick={() => handleDelete(entry)}
                    style={{ background: "transparent", border: "none", color: "var(--text-3)", display: "flex", alignItems: "center", padding: "0.2rem", flexShrink: 0 }}>
                    <X size={13} />
                  </button>
                </div>
              </div>
...
const labelStyle = {
  display: "block", color: "var(--muted)", fontSize: "0.7rem", fontWeight: 600,
